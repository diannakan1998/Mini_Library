...
const { rentalHistoryDialog } = require('./dialogs/rentalHistory');
const { currentRentalsDialog } = require('./dialogs/currentRentals');
const { libraryBooksDialog } = require('./dialogs/libraryBooks');
const { recommendationDialog } = require('./dialogs/recommendation');
const { findLibrariesDialog } = require('./dialogs/findLibraries');
const { findBooksDialog } = require('./dialogs/findBooks');
...
const RENTAL_HISTORY_DIALOG = 'rentalHistoryDialog';
const CURRENT_RENTALS_DIALOG = 'currentRentalsDialog';
const LIBRARY_BOOKS_DIALOG = 'libraryBooksDialog';
const RECOMMENDATION_DIALOG = 'recommendationDialog';
const FIND_LIBRARIES_DIALOG = 'findLibrariesDialog';
const FIND_BOOKS_DIALOG = 'findBooksDialog';
...
const RENTAL_HISTORY_INTENT = 'Rental_History';
const CURRENT_RENTALS_INTENT = 'Current_Rentals';
const LIBRARY_BOOKS_INTENT = 'Library_Books';
const RECOMMENDATION_INTENT = 'Recommendation';
const FIND_LIBRARIES_INTENT = 'Find_Libraries';
const FIND_BOOKS_INTENT = 'Find_Books';
...
const LIBRARY_NUMBER_ENTITIES = ['libraryNumber'];
const BOOK_TITLE_ENTITIES = ['bookTitle'];
...
class BasicBot {
    ...
    constructor(conversationState, userState, botConfig) {
        ...
        this.dialogs.add(new welcomeDialog(WELCOME_DIALOG, this.userProfileAccessor));
        this.dialogs.add(new GreetingDialog(GREETING_DIALOG, this.userProfileAccessor));
        this.dialogs.add(new rentalHistoryDialog(RENTAL_HISTORY_DIALOG, this.userProfileAccessor));
        this.dialogs.add(new currentRentalsDialog(CURRENT_RENTALS_DIALOG, this.userProfileAccessor));
        this.dialogs.add(new libraryBooksDialog(LIBRARY_BOOKS_DIALOG, this.userProfileAccessor));
        this.dialogs.add(new recommendationDialog(RECOMMENDATION_DIALOG, this.userProfileAccessor));
        this.dialogs.add(new findLibrariesDialog(FIND_LIBRARIES_DIALOG, this.userProfileAccessor));
        this.dialogs.add(new findBooksDialog(FIND_BOOKS_DIALOG, this.userProfileAccessor));
                ...
                switch (dialogResult.status) {
                    ...
                    case DialogTurnStatus.empty:
                        ...
                        switch (topIntent) {
                            ...
                            case RENTAL_HISTORY_INTENT:
                                console.log("Show Rental History");
                                await dc.beginDialog(RENTAL_HISTORY_DIALOG);
                                break;

                            case CURRENT_RENTALS_INTENT:
                                console.log("Show Current Rentals");
                                await dc.beginDialog(CURRENT_RENTALS_DIALOG);
                                break;

                            
                            case LIBRARY_BOOKS_INTENT:
                                console.log("Show Library Stock");
                                await dc.beginDialog(LIBRARY_BOOKS_DIALOG);
                                break;

                            case RECOMMENDATION_INTENT:

                                await dc.beginDialog(RECOMMENDATION_DIALOG);
                                break;
                                

                            case FIND_LIBRARIES_INTENT:

                                await dc.beginDialog(FIND_LIBRARIES_DIALOG);
                                break;

                            case FIND_BOOKS_INTENT:

                                await dc.beginDialog(FIND_BOOKS_DIALOG);
                                break;
    ...
    async updateUserProfile(luisResult, context) {
        ...
        if (Object.keys(luisResult.entities).length !== 1) {
            ...
            LIBRARY_NUMBER_ENTITIES.forEach(library => {
                if (luisResult.entities[library] !== undefined) {
                    userProfile.library = luisResult.entities[library][0];
                }
            });
            BOOK_TITLE_ENTITIES.forEach(bookTitle => {
                if (luisResult.entities[bookTitle] !== undefined) {
                    userProfile.bookTitle = luisResult.entities[bookTitle][0];
                }
            });
            // set the new values
